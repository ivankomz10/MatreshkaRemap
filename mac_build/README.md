...
## What changed since the first Mac build

**Source and output rates are chosen, not assumed.** The renderer used to write
every frame it read, at a fixed thirty. A sixty a second source therefore came
out twice as long as it went in. The rate of the material is now probed from
the file and can be overridden — 24, 25, 30, 48, 50 or 60 — and the written
rate is 30 or 60 beside the format. Frames are picked by time, not one for one,
so the clip keeps its length whichever pair is chosen. At equal rates nothing
is resampled and the old path is untouched.

**Fine sampling.** A checkbox beside the resolution: four samples per output
pixel instead of one. One output pixel of this mapping reaches across 1.0 to
2.0 source pixels and a single sample cannot see that far. Measured against the
same mapping supersampled sixteen times it cuts the error from 16.8 to 7.4 of
255 on hard graphic content, and costs nothing worth measuring — this stage
waits on moving frames, not on sampling. On soft photographic material the
difference is a few tenths of one level, so it earns its keep on thin type and
line work rather than everywhere.

**The window was rearranged.** The framed groups became one column read top to
bottom, the sequence table grows to the number of sequences instead of being
capped at four rows, and the strip along the bottom shows what the application
is standing on when idle and how far along it is while working. The log is
